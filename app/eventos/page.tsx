...
export default function EventosPage() {
  ...
  // Polling enquanto há evento comprando
  useEffect(() => {
  ...
  // Auto-detecta nome ao digitar URL
  useEffect(() => {
  ...
  async function iniciarBot(eventoId: string) {
    ...
    const data = await res.json()
    setBotLoading(null)
    if (!res.ok) setMsg(`Erro: ${data.error}`)
    else { setMsg('Bot iniciado! Acompanhe os logs abaixo.'); load() }
  }
          ...
          <form onSubmit={handleCriar} className="bg-gray-900 border border-gray-800 rounded-xl p-6 mb-6 space-y-5">
            ...
            <div>
              <label className="block text-sm text-gray-400 mb-1">
                Nome do evento
                {nomeLoading && <span className="ml-2 text-gray-500 text-xs">detectando...</span>}
              </label>
              <input type="text" value={nome} onChange={e => setNome(e.target.value)}
                placeholder="Detectado automaticamente pela URL"
                className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-orange-500 placeholder-gray-600" />
            </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex items-center gap-2">
                  <select value={tipoGlobal} onChange={e => setTipoGlobal(e.target.value as 'entrada' | 'geral')}
                    ...
                    <option value="entrada">Ingresso com horário limite</option>
              ...
              {contas.length === 0 ? (
                <p className="text-gray-500 text-sm">Nenhuma conta cadastrada. <a href="/contas" className="text-orange-400 hover:underline">Cadastrar agora</a></p>
              ) : (
                  ...
                  {contas.map(c => {
                    ...
                    return (
                      <div key={c._id} onClick={() => toggleConta(c._id)}
                        className={`flex items-center justify-between p-3 rounded-lg border transition-colors cursor-pointer ${sel ? 'border-orange-500 bg-orange-500/10' : 'border-gray-700 bg-gray-800 hover:border-gray-600'}`}>
                ...
                <div key={ev._id} className="bg-gray-900 border border-gray-800 rounded-xl p-5">
                  <div className="flex items-start justify-between">
                    ...
                    <div className="flex flex-col gap-2 shrink-0">
                      ...
                      {(ev.status === 'comprando' || ev.status === 'finalizado' || ev.status === 'erro') && (
                        <Link href={`/logs?eventoId=${ev._id}`}
                          className="bg-gray-700 hover:bg-gray-600 text-white text-sm px-3 py-1.5 rounded-lg transition-colors text-center">
                          📋 Ver Logs
                        </Link>
                      )}
                      {(ev.status === 'finalizado') && (
                        <Link href="/qrcodes"
                          className="text-orange-400 hover:underline text-sm text-center">
                          Ver QR Codes →
                        </Link>
                      )}
                    </div>
